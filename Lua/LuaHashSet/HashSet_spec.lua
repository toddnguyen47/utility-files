insulate("HashSet Test | ", function()
    ...
    setup(function() hashSet = require("HashSet"):new() end)

    before_each(function() hashSet:clear() end)

    randomize()

    test("isEmpty()", function() assert.is_true(hashSet:isEmpty()) end)
    ...
    test("add multiple DUPLICATES; size should be 1, isEmpty should be false",
         function()
    ...
    test(
        "add multiple non-duplicates; size should be 1, isEmpty should be false",
        function()
            math.randomseed(os.time())
            local numTimes = math.random(10, 20)
    ...
    test("add 1 then remove 1, size == 0 and hashset should be empty",
         function()
        hashSet:add("elem1")
    ...
    test(
        "add 1 then remove 1 then readd it, size == 1 and hashset should not be empty",
        function()
            hashSet:add("elem1")
            hashSet:remove("elem1")
            hashSet:add("elem1")
    ...
    test(
        "Remove from empty list; Size should still be 0, list should still be empty",
        function()
            hashSet:remove("elem1")
            assert.is_true(hashSet:isEmpty())
            assert.is.equal(0, hashSet:size())
