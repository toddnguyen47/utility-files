...
public class GetTimeInUtcTest {
  ...
  public void test_GivenNow_WhenGettingTime_ThenTimeShouldBeWithinTwoMinInterval() {
    ...
    final OffsetDateTime currentTimeConverted =
        OffsetDateTime.parse(currentTime, GetTimeInUtc.DATE_TIME_FORMATTER);
    final boolean now1WithinInterval =
        now1.isBefore(currentTimeConverted) || now1.isEqual(currentTimeConverted);
    final boolean now2WithinInterval = now2.isAfter(currentTimeConverted);
    Assert.assertTrue(
        String.format(
            "now1WithinInterval: %s, now2WithinInterval: %s",
